...
1. VPN da LES ligada, e `ssh root@IP-DA-VPS` funcionando.
2. `npm run publicar -- root@IP-DA-VPS` — instala Node, usuário, pastas e serviço, e cria
   `/etc/tscquestoes/ambiente` já com um `AUTH_SECRET` novo.
3. Na VPS, preencha o arquivo (sem colar esses valores em chat nenhum):
   `nano /etc/tscquestoes/ambiente`
...
```bash
npm run release -- https://SEUNOME.cloud.deploy.uespi.br
npm run publicar -- root@IP-DA-VPS
```
